...
def prompt(name: str, username: str, basic_user_info: str, chunk: str) -> str:
    return f'''Here is an example of the output for another character, Eliza:
{OUTPUT_FORMAT}
...
A brief bio for {name} (1 paragraph)
5-10 interesting facts about {name} (lore)
3-5 adjectives that describe {name}'s posts
3-5 specific topics {name} is interested in
3-5 stylistic directions for how {name} speaks which are very specific to this user's writing style
3-5 stylistic directions for how {name} chats in DMs or back-and-forth conversations, again only capturing the specific nuances of this user's writing style
3-5 stylistic directions for how {name} writes posts (post), specific to how the user writes and formats posts and presents information
...
class Style(BaseModel):
    all: List[str] = Field(description="General style directions")
    chat: List[str] = Field(description="Chat style directions")
    post: List[str] = Field(description="Post style directions")
...
class FormattedBio(BaseModel):
    bio: str = Field(description="A brief bio for the user")
    lore: List[str] = Field(description="5-10 interesting facts about the user")
    adjectives: List[str] = Field(description="3-5 adjectives that describe the user")
    topics: List[str] = Field(description="3-5 specific topics the user is interested in")
    style: Style = Field(description="3-5 stylistic directions for how the user speaks")
    messageExamples: List[List[Message]] = Field(description="Examples of messages back and forth with imaginary users")
    postExamples: List[str] = Field(description="Examples of posts which the user has written")
